...
| `dispositivos` | 35 columnas — `id`, `uuid`, `nombre`, `dominio` (FK), `agente` / `modelo` / `producto` / `transceptor` / `chip` / `adopcion` (FK), `mac`, `ip`, `senal`, `firmware`, `serial`, `identidad`, `llave`, `habilitado` (tinyint 1/0, `NOT NULL`) / `enlace` / `adoptado` / `monitoreo` (smallint booleanos), contadores, fechas y monitoreo (ver `db/schema.sql`). El listado las expone bajo nombres de UI (`uid` = `uuid`, `tipo` = nombre del modelo, `ubicacion` = `coordenadas`, `estado` derivado de `habilitado`/`enlace`); los modales de Consultar y Editar hablan el esquema real |
