...
/**
 * Piezas compartidas del ciclo de invitaciones de la app.
 *
 * Las usan el endpoint del alta (api/invitaciones.php) y las tres paginas
 * publicas de invitacion/ (ver, aceptar, rechazar), que no exigen sesion.
 *
 * ES EL MISMO CICLO QUE EL DEL PANEL (panel/lib/invitaciones.php), sobre las
 * MISMAS tablas y con el MISMO canal de salida (correo por Databox v4). Lo que
 * cambia son dos cosas, y ninguna es cosmetica:
 *
 *   1. EL ENLACE APUNTA A `app.reactor.com.ar/invitacion/`, no a `panel.`. La
 *      invitacion la emite un usuario de la app para que otro OPERE los equipos
 *      del dominio; mandarlo al panel lo dejaria en una pantalla a la que ni
 *      siquiera puede entrar (al panel solo entra `perfiles.tipo = 'A'`).
 *   2. EL PERFIL QUE SE CREA AL ACEPTAR ES DE OPERADOR, con `operacion` e
 *      `invitacion` en 1 y `facturacion` en 0. La regla vive entera en
 *      lib/perfiles.php; aca se nombra porque es lo que explica el punto 1.
 *
 * De paso, esta ruta es la MISMA que usaba el legacy (`reactor-app/invitacion/`)
 * y que quedo sin servir cuando el DNS de `app.reactor.com.ar` paso al server
 * nuevo: los enlaces viejos que anden dando vuelta vuelven a resolver, y con la
 * semantica correcta — el legacy tambien daba de alta operadores.
 *
 ...
 * `appBaseUrl()` vive en lib/base_url.php y fija el host en produccion: el
 * enlace viaja dentro de un correo, asi que derivarlo del `Host` de la request
 * dejaria que un `Host` falseado mandara a los invitados a otro dominio.
 */
function invitacionUrl(string $uuid): string
{
    return appBaseUrl() . '/invitacion/?uid=' . rawurlencode($uuid);
}
