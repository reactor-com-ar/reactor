 ...
 *
 * COPIA IDENTICA de panel/lib/databox.php y app/lib/databox.php. Es el MISMO
 * canal de salida —mismo endpoint, misma apikey, mismos slugs— y tiene que
 * serlo: la difusion que se manda desde cloud sale por la misma cuenta SES y
 * con la misma identidad visual que la invitacion del panel y la de la app.
 * Esta duplicado y no incluido porque las tres apps no comparten docroot, igual
 * que `habilitado.php`, `permisos.php` y `usuarios_alta.php`. Si se cambia una
 * copia, hay que cambiar las otras dos.
 *
 * EN CLOUD LO USA EL MODULO DIFUSION (api/difusion.php), que manda un correo
 * por destinatario. Es la unica diferencia de uso con las otras dos copias, que
 * mandan de a uno: aca la funcion se llama en un bucle acotado por lote, y por
 * eso importa que NUNCA lance (devuelve ['ok' => false, ...]) — un destino que
 * rebota tiene que marcar esa fila como fallida y seguir con la siguiente, no
 * cortar el lote entero.
 *
 * En panel y app reemplaza el camino que usaba el legacy para las invitaciones
