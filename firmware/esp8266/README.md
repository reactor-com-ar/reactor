# Firmware ESP8266 — Reactor

Firmware base para dispositivos basados en **ESP8266** (NodeMCU, Wemos D1
mini, ESP-12E, etc.) que forman parte de la plataforma IoT **Reactor**. Estos
dispositivos se administran desde el panel web en [`cloud/`](../../cloud/) y
reportan datos vía MQTT.

Sketch principal: [`esp8266.ino`](esp8266.ino).
...
1. **Boot.** Inicializa el puerto serial (115200 baud), monta **LittleFS**
   (lo formatea y reintenta si el mount falla) y calcula su `uid` a partir
   de la MAC del chip (`WiFi.macAddress()` sin `:`).
...
Una sola escritura en flash = un solo punto de verdad consistente. Mientras
el documento se mantenga por debajo de ~8 KB (hoy es ~1.5 KB con todas las
secciones llenas), no hay razón técnica para partirlo.
...
| `channels[i].pin` | int | GPIO del ESP8266 al que está conectado el módulo (no el "D*N*" de la placa). |
...
| `input` | `pinMode(INPUT)` | Sensor digital con pull-down externo. |
| `input_pullup` | `pinMode(INPUT_PULLUP)` | Botón / contacto seco contra GND. |
| `pwm` | `pinMode(OUTPUT)` + `analogWrite(initial)` | Atenuación de LED, control de velocidad, etc. Rango ESP8266: 0–1023. |
| `unused` | (nada) | Reserva el slot del canal pero no toca el hardware. |

Reglas y límites de hardware:

- Máximo **8 canales** (`MAX_CHANNELS` en el `.ino`). Si el payload trae
  más, los sobrantes se truncan con warning.
- Los pines **6–11 se ignoran** porque están conectados al flash interno
  del ESP8266; configurarlos cuelga el chip. El firmware loguea
  `[channel] #N pin X ignorado` y sigue.
- Pines con consideraciones de boot: GPIO 0 (boot mode), GPIO 2 (boot mode,
  LED en muchas placas), GPIO 15 (debe estar LOW al boot), GPIO 16 (sin
  interrupt, sin PWM). Son usables pero hay que conocer las restricciones.
...
1. Editar la sección `users` de `firmware/esp8266/data/config.json`
   localmente y subir el archivo con el plugin **arduino-littlefs-upload**
...
1. **Editar defaults y flashear.** Modificar las constantes `DEFAULT_*` al
   principio de [`esp8266.ino`](esp8266.ino), compilar y subir. En el primer
...
2. **Subir `data/config.json` por LittleFS.** Crear el archivo en
   `firmware/esp8266/data/config.json` con los valores reales del equipo y
   subirlo con el plugin **arduino-littlefs-upload** del Arduino IDE. Queda
   en el FS del dispositivo sin tocar el sketch — recomendado cuando se
   aprovisionan varios dispositivos con el mismo binario pero distintas
   credenciales.
...
3. *Boards Manager* → instalar **esp8266 by ESP8266 Community**.
4. *Tools → Board* → seleccionar la placa real (típicamente *NodeMCU 1.0
   (ESP-12E)*).
5. *Tools → Flash Size* → elegir un esquema que reserve espacio para
   **FS (LittleFS)**, p. ej. `4MB (FS:1MB OTA:~1019KB)`.
...
`ESP8266WiFi`, `ESP8266WebServer` y `LittleFS` ya vienen con el core ESP8266,
no requieren instalación adicional.

### Plugin opcional

- **arduino-littlefs-upload** — para subir el contenido de `data/` al FS del
  dispositivo sin recompilar el sketch.
