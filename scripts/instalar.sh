...
for name in reactor reactor-db; do
...
if ! docker compose -p reactor up -d --build; then
    ...
    echo ""
    echo -e "${YELLOW}--- docker logs reactor (ultimas 50 lineas) ---${NC}"
    docker logs --tail 50 reactor 2>&1 || true
    echo ""
    echo -e "${YELLOW}--- docker logs reactor-db (ultimas 50 lineas) ---${NC}"
    docker logs --tail 50 reactor-db 2>&1 || true
    exit 1
fi
...
if [ -d "$migrations_dir" ]; then
    ...
    for m in "$migrations_dir"/*.sql; do
        ...
        name=$(basename "$m")
        echo "    $name"
        if ! docker cp "$m" "reactor-db:/tmp/migration.sql" > /dev/null; then
            echo -e "${RED}ERROR copiando $name al contenedor reactor-db${NC}"
            exit 1
        fi
        if ! docker exec reactor-db sh -c 'mysql -uroot -proot reactor_dev < /tmp/migration.sql'; then
            echo -e "${RED}ERROR aplicando $name${NC}"
            exit 1
        fi
    done
    docker exec reactor-db rm -f /tmp/migration.sql > /dev/null || true
fi
...
if $ok; then
    ...
    echo -e "${YELLOW}==> Stack arriba, pero la app aun no responde. Revisa logs con: docker logs reactor${NC}"
fi
...
echo ""
echo "  Logs    : docker logs -f reactor"
