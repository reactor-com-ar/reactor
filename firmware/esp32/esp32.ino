...
//
// Port del firmware de ESP8266. Las diferencias respecto al sketch hermano
// están localizadas en:
//   - includes (WiFi.h / WebServer.h en lugar de los ESP8266WiFi/WebServer)
//   - tipo del server global (WebServer en lugar de ESP8266WebServer)
//   - montaje de LittleFS con flag formatOnFail = true
//   - resolución de PWM seteada a 10 bits para mantener el rango 0..1023
//     (en arduino-esp32 v3+ el default de analogWrite() es 8 bits / 0..255)
...
void cargarConfig() {
  // En arduino-esp32, LittleFS.begin(true) formatea automáticamente si el
  // mount falla. Más simple que el patrón format-and-retry de ESP8266.
  if (!LittleFS.begin(true)) {
    Serial.println("[fs] fallo al montar LittleFS (incluso con auto-format)");
  }
...
void aplicarChannels() {
  for (int i = 0; i < channelCount; i++) {
    ...
    // GPIO 6-11 están conectados a la flash SPI interna en módulos WROOM/WROVER:
    // tocarlos cuelga el chip. (Módulos PICO-D4 y variantes -V3 sí los liberan,
    // pero por defecto asumimos el chip WROOM más común.)
    if (c.pin >= 6 && c.pin <= 11) {
      Serial.printf("[channel] #%d pin %d ignorado (reservado para flash SPI)\n",
                    c.id, c.pin);
      continue;
    }
    ...
    if (c.mode == "output") {
      ...
      pinMode(c.pin, OUTPUT);
      analogWriteResolution(c.pin, 10);  // alinear con el rango ESP8266 0..1023
...
void setup() {
  ...
  delay(100);
  Serial.println();
  Serial.printf("=== Reactor ESP32 v%s ===\n", FIRMWARE_VERSION);
